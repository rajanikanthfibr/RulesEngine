...
def get_transactions(company_id):
    ...
    try:
        ...
        for transaction_data in transactions:
            j = transaction_data[1]
            transactions_rev.append({
                "accountId": j['account_id'],
                "amount": abs(j['amount']),
                "date": j['transaction_date'],
                "description": j['description'],
                "transactionClass": 'debit' if j['amount'] < 0 else 'credit',
...
def get_trusso_sales(company_no):  # Trusso
    ...
    url = 'https://uat-uk-api.experian.com/ts-categorization-svc/transactions/v4/uk/b2b.json'
    headers = {'Authorization': bearer_token,
               'Accept': 'application/json',
    ...
    try:
        for doc in j['transactions']:
            ...
            if (doc['predictedCategory'] in ['INC-I0001', 'INC-I0003']) and (
                    trans_date >= datetime.now().date() - timedelta(days=366)):
